...
function findIndexByProperty(arr, name) {
  ...
  const a = arr.findIndex((item) => item.name === name);
  return a;
...
function findLastEvenNumber(arr) {
  ...
  const a = arr.findLast((num) => num % 2 === 0);
  if (a !== undefined) {
    return a;
  } else {
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const a = arr.flat();
  const b = a.map((i) => i ** 2);
  const c = b.filter((i) => i > threshold);
  return c;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const a = arr.slice(-n);
  const b = a.reduceRight((ac, cur) => ac + cur, 0);
  a.forEach((el) => {
    console.log(el);
  });
  return b;
...
function wordLengths(arr) {
  ...
  const a = arr.map((arr) => arr.length);
  return a;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо 'Вхідний параметр має бути масивом'
...
function capitalizeWords(arr) {
  ...
  const capitalizedArray = arr.map((word) => {
    ...
    const a = word[0].toUpperCase() + word.slice(1);
...
function findLastNegativeNumberIndex(arr) {
  ...
  const a = arr.findLastIndex((num) => num < 0);
  return a;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо -1
